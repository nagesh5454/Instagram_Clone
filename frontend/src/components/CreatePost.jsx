...
const CreatePost = ({ open, setOpen }) => {
  ...
  const createPostHandler = async (e) => {
    ...
    const formData = new FormData();
    formData.append("caption", caption);
    if (file) formData.append("image", file); // Ensure actual file is appended

    try {
      setLoading(true);

      // API request to create a new post
      const res = await axios.post('https://instagram-clone-a9ip.onrender.com/api/v1/post/addpost', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
        withCredentials: true,
      ...
      if (res && res.data && res.data.success) {
        ...
        toast.error("Something went wrong. Please try again."); // Handle unexpected response
      }
    } catch (error) {
